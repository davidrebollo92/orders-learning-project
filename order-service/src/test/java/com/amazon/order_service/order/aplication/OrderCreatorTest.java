...
class OrderCreatorTest {
    ...
    @Test
    void create_reservesStockSavesOrderAndPublishesEvent() {
        UUID productId = UUID.randomUUID();
        int quantity = 2;
        Money price = new Money(new BigDecimal("10.00"));
        ProductData productData = new ProductData(productId, price);
        Order savedOrder = Order.create(productId, quantity, price);
        ...
        verify(stockReservationGateway).reserve(any(UUID.class), eq(productId), eq(quantity));
